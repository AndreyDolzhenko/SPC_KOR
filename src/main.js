...
const getUsersAccaunt = () => {
	if (param === null) {
		...
		userData.innerHTML = `${searchParams.get("fio_person")}<br>${searchParams.get("position_person")}`;
	}
};
...
const changeOfDivision = (newDivision) => {
	...
	if (newDivision != keySubDivision) {
		...
		for (clientStatus of listScript.children) {
			...
			if (subDivisionAndstatusScripts[keySubDivision]["status-scripts"][clientStatus.id] != undefined) {
		...
		btnSubDivision[index].addEventListener("click", (event) => {
			...
			for (let index = 0; index < btnSubDivision.length; index++) {
				...
				if (btnSubDivision[index].value == event.target.value) {
					btnSubDivision[index].style.background = "linear-gradient(45deg, black, transparent)";
...
for (subDivision in subDivisionAndstatusScripts) {
	...
	newDivSubDivision.className = i > 0 ? "btn_sub-division" : "btn_sub-division btn_selected";
...
for (let index = 0; index < buttonPrep.length; index++) {
	buttonPrep[index].addEventListener('click', function (event) {
		const doNotMakeContactBlockId = document.getElementById('doNotMakeContactBlock');
	...
	for (let index = 0; index < el.length; index++) {
		el[index].addEventListener("click", function (event) {
			...
			if (event.target.className == "presentationsUrgentScripts") {
				textOutput.innerHTML = `<b>${clientName}</b>` + ', ' + presentationsUrgentScripts[event.target.innerText];
			...
			if (event.target.className == "presentations") {
				for (ObjPresentations of arrObjPresentations) {
					if (ObjPresentations["title"] == event.target.innerHTML) {
						textOutput.innerHTML = ObjPresentations["sub_presentations"] == true ? '<span id="OSS">' + ObjPresentations['content'] + '</span>' : textOutput.innerHTML = ObjPresentations['content'];
					}
				};
				// вешаем попап на сервисы
				for (let index = 0; index < Services.length; index++) {
...
listScript.addEventListener("click", function (event) {
	...
	if (event.target.id != 'list_script') {
		...
		i = 0;
		if (subDivisionAndstatusScripts[keySubDivision]['status-scripts'][event.target.id] != undefined) {
			let liId = 0;
			for (scriptPoint of subDivisionAndstatusScripts[keySubDivision]['status-scripts'][event.target.id]) {
				...
				newLiScript.innerHTML = i % 2 == 0 ? `<b>${clientName}, </b>` + scriptPoint : scriptPoint[0].toUpperCase() + scriptPoint.slice(1);
	...
	send.addEventListener("click", (event) => {
		...
		if (dataToSend.progress != 0 && userData.textContent != "Имя оператора" && x != 0) {
